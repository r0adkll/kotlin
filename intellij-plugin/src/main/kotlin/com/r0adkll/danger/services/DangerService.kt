...
class DangerService(
  ...
  suspend fun load() {
    ...
    val pluginVersion = PluginManager.getInstance()
      .findEnabledPlugin(PluginId.getId("com.r0adkll.danger"))
      ?.version
...
//    }
  ...
  private suspend fun copyDangerJar(version: String) = withContext(Dispatchers.IO) {
    val packagedJar = DangerService::class.java
      .classLoader
      .getResourceAsStream("jar/danger-kotlin.jar")
  ...
  private fun getDangerSourceJarPath(version: String): Path {
    return project.basePath?.toNioPathOrNull()

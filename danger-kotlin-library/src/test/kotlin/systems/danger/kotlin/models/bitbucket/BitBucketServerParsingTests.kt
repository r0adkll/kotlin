...
class BitBucketServerParsingTests {
  ...
  fun testItParsesTheBitBucketPullRequest() {
    with(bitBucketServer.pullRequest) {
      ...
      val expectedReviewer =
        BitBucketServerReviewer(
  ...
  fun testItParsesTheBitBucketCommits() {
    with(bitBucketServer.commits) {
      ...
      val expectedParent =
        BitBucketServerCommitParent("c62ada76533a2de045d4c6062988ba84df140729", "c62ada76533")
      val expectedCommit =
  ...
  fun testItParsesTheBitBucketComments() {
    with(bitBucketServer.comments) {
      ...
      val expectedProperty = BitBucketServerCommentInnerProperties(1, listOf())
      val expectedCommentDetail =
        BitBucketServerCommentDetail(
          10,
          23,
          commentText,
          expectedUser,
  ...
  fun testItParsesTheBitBucketActivities() {
    with(bitBucketServer.activities) {
      ...
      val expectedActivity =
        BitBucketServerActivity(61, 1519442356495, expectedUser, "RESCOPED", null)

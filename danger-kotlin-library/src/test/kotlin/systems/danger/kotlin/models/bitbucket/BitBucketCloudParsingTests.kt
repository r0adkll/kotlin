    ...
    with(bitBucketCloud.pullRequest) {
      ...
      val expectedDestination =
        BitBucketCloud.MergeRef(
          branch = BitBucketCloud.MergeRef.Branch(name = "destination"),
          commit = BitBucketCloud.MergeRef.Commit(hash = "test"),
          repository = BitBucketCloud.Repo(uuid = "test", name = "test", fullName = "test"),
        )
      assertEquals(expectedDestination, destination)

      val expectedSource =
        BitBucketCloud.MergeRef(
          branch = BitBucketCloud.MergeRef.Branch(name = "source"),
          commit = BitBucketCloud.MergeRef.Commit(hash = "test"),
          repository = BitBucketCloud.Repo(uuid = "test", name = "test", fullName = "test"),
        )
    ...
    with(bitBucketCloud.comments) {
      val expectedComment =
        BitBucketCloud.Comment(
          id = 1,
          content = BitBucketCloud.Content(html = "test", markup = "markdown", raw = "test"),
